...
class ObjectDetectionApp:
    def __init__(self, root):
        ...
        self.save_button = tk.Button(model_tab, text="Clear Model", command=self.clear_model)
        self.save_button.pack(pady=10)

        # Adding YOLO Checkbox
        self.yolo_checkbox = ttk.Checkbutton(model_tab, text="Use YOLO", command=self.toggle_yolo)
        self.yolo_checkbox.pack(pady=10)

        # Data tab
        self.patch_button = tk.Button(data_tab, text="Generate Patches", command=self.make_patches)
        self.patch_button.pack(pady=10)

        self.splat_button = tk.Button(data_tab, text="Generate splattered images from kernels", command=self.splat_images)
        ...
        # Annotations tab
        self.create_xml_button = tk.Button(annotations_tab, text="Create annotation xml files via model", command=self.create_annotation_files)
        self.create_xml_button.pack(pady=10)


        self.create_xml_button = tk.Button(annotations_tab, text="Convert xml annotation to yolo", command=self.convert_xml_to_yolo)
        self.create_xml_button.pack(pady=10)

        self.filter_xmls_button = tk.Button(annotations_tab, text="Filter objects from xml", command=self.filter_xml_objects)
        self.filter_xmls_button.pack(pady=10)

        self.filterout_xmls_button = tk.Button(annotations_tab, text="Remove objects from xml", command=self.filterout_xml_objects)
        self.filterout_xmls_button.pack(pady=10)

        self.rename_xmls_button = tk.Button(annotations_tab, text="Rename objects in xml", command=self.rename_xml_objects)
        ...
        self.test_button = tk.Button(test_model_frame, text="Test Model on Random Images", command=self.test_model_on_random_images)
    ...
    def filter_xml_objects(self):
        ...
        keep_list = tk.simpledialog.askstring("Enter the tags to keep space separated", prompt="Enter the tags to keep space separated",
    ...
    def filterout_xml_objects(self):
        source_folder = filedialog.askdirectory(title="Select folder containing xmls")
        keep_list = tk.simpledialog.askstring("Enter the tags to remove space separated", prompt="Enter the tags to remove space separated",
                                              initialvalue="DCS INST SP")
    ...
    def rename_xml_objects(self):
        source_folder = filedialog.askdirectory(title="Select folder containing xmls")
        rename_list = tk.simpledialog.askstring("Enter the tags to rename use arrows",prompt="Enter the tags to rename use arrows -> and space",
    ...
    def create_annotation_files(self):
        ...
        min_score = tk.simpledialog.askinteger("Minimum Score percent", "Enter a Minimum Score percent:", initialvalue=75)/100
    ...
    def convert_xml_to_yolo(self):
        ...
        data_split = tk.simpledialog.askfloat("Data split", "Enter ratio of that data goes to training", initialvalue=0.9)
    ...
    def splat_images(self):
        ...
        #splat_gui_window.protocol("WM_DELETE_WINDOW", lambda: self.on_close(splat_gui_window))
    ...
    def train_model(self):
        ...
        if self.use_yolo:
            ...
            yaml_path = filedialog.askopenfilename(title='Select the data.yaml file', filetypes=[("YAML files", "*.yaml")])
            imgsz = tk.simpledialog.askinteger("image size", "Target image size for training. All images are resized to this dimension before being fed into the model. Affects model accuracy and computational complexity.",
                                               initialvalue=1300)
    ...
    def load_pretrained_model(self):
        model_path = filedialog.askopenfilename(title="Select pretrained model file",filetypes=[("Model files", "*.pth;*.pt")])
        if model_path:
...
#tk.simpledialog.askinteger("Scale percent", "Enter a scale percent 1-100:", initialvalue=100)
